...
if [[ "$OS_ID" =~ ^(rocky|almalinux|rhel|centos|ol)$ ]] || [[ "$OS_LIKE" =~ (rhel|fedora|centos) ]]; then
  printf '[INFO] Detected RHEL family (%s). Delegating to install_server_rocky.sh\n' "${OS_ID:-unknown}"
  exec "${SCRIPT_DIR}/install_server_rocky.sh" "$@"
fi

if [[ "$OS_ID" != "ubuntu" && "$OS_ID" != "debian" && ! "$OS_LIKE" =~ (ubuntu|debian) ]]; then
  printf '[ERROR] Unsupported distro: %s. Use Ubuntu/Debian or Rocky/Alma/RHEL/CentOS.\n' "${OS_ID:-unknown}" >&2
  exit 1
fi
...
DB_CONN_MAX_LIFETIME_MINUTES="${DB_CONN_MAX_LIFETIME_MINUTES:-30}"
DB_CONN_MAX_IDLE_MINUTES="${DB_CONN_MAX_IDLE_MINUTES:-5}"
...
Options:
  ...
  --db-max-open <int>           DB_MAX_OPEN_CONNS
  --db-max-idle <int>           DB_MAX_IDLE_CONNS
  --db-max-lifetime <minutes>   DB_CONN_MAX_LIFETIME_MINUTES
  --db-max-idle-time <minutes>  DB_CONN_MAX_IDLE_MINUTES
  --redis-url <url>             RATE_LIMIT_REDIS_URL
  --redis-prefix <prefix>       RATE_LIMIT_REDIS_PREFIX
  --api-rate-limit <int>        API_RATE_LIMIT_PER_MINUTE
  --api-rate-window <seconds>   API_RATE_LIMIT_WINDOW_SECONDS
  --api-timeout <seconds>       API_REQUEST_TIMEOUT_SECONDS
  --max-header-bytes <bytes>    MAX_HEADER_BYTES
  --storage-provider <provider> STORAGE_PROVIDER (default: local)
  --storage-local-path <path>   STORAGE_LOCAL_PATH
  --reminder-poll <seconds>     REMINDER_POLL_SECONDS
...
Examples:
  ...
  ./deploy/install_server.sh --app-env production --jwt-secret "very-strong-secret-xxx"
EOF
}
  ...
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --db-max-idle)
        ...
        ;;
      --db-max-lifetime)
        DB_CONN_MAX_LIFETIME_MINUTES="$2"
        shift 2
        ;;
      --db-max-idle-time)
      ...
      --api-rate-limit)
        ...
        ;;
      --api-rate-window)
        API_RATE_LIMIT_WINDOW_SECONDS="$2"
        shift 2
        ;;
      --api-timeout)
...
install_docker() {
  ...
  if [[ "$distro" != "ubuntu" && "$distro" != "debian" ]]; then
    die "Unsupported distro for automatic Docker install: ${distro}. Use Ubuntu/Debian."
  fi

  as_root install -m 0755 -d /etc/apt/keyrings
  curl -fsSL "https://download.docker.com/linux/${distro}/gpg" | as_root gpg --dearmor -o /etc/apt/keyrings/docker.gpg
...
sync_project() {
  ...
  as_root rsync -av --delete-after \
    ...
    --exclude "backend/.env" \
    "${REPO_DIR}/" "${INSTALL_DIR}/" || die "Sync failed. Check permissions or disk space."
...
DB_CONN_MAX_IDLE_MINUTES=${DB_CONN_MAX_IDLE_MINUTES}
JWT_SECRET=${JWT_SECRET}
RECEIPT_SCAN_SECRET=${RECEIPT_SCAN_SECRET}
...
run_backend_bootstrap() {
  ...
  if [[ ! -f "${INSTALL_DIR}/backend/go.mod" ]]; then
    die "go.mod not found in ${INSTALL_DIR}/backend. Sync may have failed or project structure is invalid."
  fi
  ...
  log "Applying tenant-specific migrations to all schemas"
  as_app "cd '${INSTALL_DIR}/backend' && DATABASE_URL='${DATABASE_URL}' /usr/local/go/bin/go run ./scripts/migrate_tenants.go"
...
install_fail2ban() {
  log "Installing and configuring Fail2Ban for security"
  as_root apt-get update
  as_root apt-get install -y fail2ban
...
server {
    ...
    location /api/ {
        ...
        # Increase buffers for large JWT cookies
...
configure_web_firewall() {
  ...
  as_root ufw allow "${WEB_PORT}/tcp" >/dev/null 2>&1 || true
}
...
- URL:          http://<server-ip>:${WEB_PORT}/admin
- Secret:       ${JWT_SECRET}
  (This secret is required to login to the System Administrator dashboard)
...
- Service stats: systemctl status pekan-api pekan-worker --no-pager
...
load_existing_config() {
  ...
  if [[ -f "$env_file" ]]; then
    ...
    if [[ -n "${DATABASE_URL}" ]]; then
      INSTALL_DB=0 # Assume DB is already set up if URL exists and no flags provided
    fi
  fi
}
