-- Backfill permission mapping for existing tenants/roles after new finance features were introduced.
-- This prevents legacy environments from failing with 403 on newer endpoints (settings + attachments).

-- Ensure required permission rows exist (idempotent).
...
-- Ensure finance.settings module + features exist in catalog (idempotent).
...
-- Auto-grant all finance permissions to role code "owner" for existing tenants.
...
-- Ensure tenant feature toggles for settings exist for active tenants without overriding existing toggles.
