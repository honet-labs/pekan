                  ...
                  {status?.bot_phone_number ? (
                    <li>
                      ...
                      <div style={{ margin: '8px 0' }}>
                        <a 
                          href={`https://wa.me/${status.bot_phone_number.replace(/\+/g, "")}?text=!login%20${otpCode}`}
                          target="_blank"
                    ...
                    <li>Buka WhatsApp Anda.</li>
                  )}
                  <li>Kirim pesan berikut ke Bot WhatsApp Anda: 
                    <div style={{ display: 'flex', alignItems: 'center', gap: '8px', margin: '6px 0', background: 'var(--surface)', padding: '6px 12px', borderRadius: '6px', border: '1px solid var(--border)', fontFamily: 'monospace', fontSize: '0.95rem' }}>
                      <span style={{ flex: 1 }}>!login {otpCode}</span>
                      <button 
                        type="button" 
                        onClick={() => handleCopy(`!login ${otpCode}`, "Perintah login berhasil disalin!")}
